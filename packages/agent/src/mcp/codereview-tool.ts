...
export const codeReviewInputSchema = z.object({
    ...
    sessionId: z
        .string()
        .optional()
        .describe("Session ID for resuming a suspended review"),
    ...
    relevant_files: z
        .array(z.string())
        .optional()
        .describe("Specific files to review"),
    review_type: z
...
export const codeReviewTool = {
    ...
    execute: async (args: CodeReviewInput) => {
        try {
            let sessionId =
                args.sessionId ||
                `review-${Date.now()}-${Math.random().toString(36).substring(7)}`;
            ...
            if (result.status === "suspended") {
                ...
                const suspendedStepPath = result.suspended?.[0] || [
                    "code-review",
                ];
                ...
                const stepState =
                    result.steps?.["code-review"]?.suspendData?.currentState ||
                    {};
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    error:
                                        result.error ||
                                        "Unknown error occurred during code review.",
                                },
                                null,
                                2,
                            ),
